...
class Sniffer(Thread):
    ...
    def print_packet(self, packet):
        print("[!] A packet was received from the switch: ")
        packet.show()
...
def send_packet(iface, addr, rank):
    print("Sending on interface %s to %s, rank %d\n" % (iface, str(addr), rank))
    pkt =  Ether(src=get_if_hwaddr(iface), dst='00:00:01:00:01:02')
    pkt = pkt /IP(dst=addr, tos=rank)
    pkt = pkt / Raw(b'Hello, world!')
...
def main():
    ...
    try:
        while True:
            send_packet(iface, addr, int(input('next_rank: ') or 0))
